...
async def file_context(connection, file_ids: list[UUID], conversation_id: UUID | None = None) -> list[dict]:
    ...
    return [
        {'role': 'system', 'content': 'Содержимое прикреплённых файлов уже извлечено сервером и включено в последнее сообщение пользователя вместе с вопросом. '
         'При наличии этих данных анализируй их; не проси повторно прикрепить файл или переписать его содержимое. '
         'Если для ответа данных недостаточно, укажи, чего именно не хватает. '
